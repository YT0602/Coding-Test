...
<p>최단거리 찾는 BFS 문제지만 먹이 선정 기준때문에 까다로운 문제였다.</p>
<p>최단거리 먹이들을 리스트로 새로 만들고 리스트를 정렬하는 함수를 추가로 만들고 정렬한 뒤</p>
<p>가장 우선인 좌표를 반환하여 BFS함수를 재귀 호출하여 다시 실행하였다.</p>