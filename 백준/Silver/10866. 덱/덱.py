...
for _ in range(N):
    ...
    if name[0] == 'push_back':  # 명령이 push_back이면
        st.append(int(name[1]))  # 덱에 숫자 추가
    if name[0] == 'push_front':  # 명령이 push_front이면
        st.appendleft(int(name[1]))  # 덱에 숫자 추가
    if name[0] == 'pop_front':  # 명령이 pop_front이면 원소가 없으면 -1, 아니면 맨 앞 원소 빼내기
        if len(st) == 0:
            print(-1)
        else:
            print(st.popleft())
    if name[0] == 'pop_back':  # 명령이 pop_back이면 원소가 없으면 -1, 아니면 맨 앞 원소 빼내기
        if len(st) == 0:
            print(-1)
        else:
            print(st.pop())
